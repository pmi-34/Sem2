...
procedure Simplify2(var Root : PNode);
var
  Q,T : PNode;
begin
  if (Root <> nil) then begin
    ...
    if ((Root^.T = Operation) and (Root^.Data = ord('/'))) and
       ((Root^.Left <> nil) and (Root^.Left^.T = Operation)) then
         if ((Root^.Left^.Data = ord('+')) or (Root^.Left^.Data = ord('-')))
         then begin
           New(Q);
           Q^.T := Operation;
           Q^.Data := ord('/');
           ...
           Root^.Right := Q;
         end else if (Root^.Left^.Data = ord('/')) then begin
           New(Q);
           Q^.T := Operation;
           Q^.Data := ord('*');
